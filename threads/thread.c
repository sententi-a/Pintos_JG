...
init_thread (struct thread *t, const char *name, int priority) {
	...
	/* weel1-4*/
	t->priority_before=priority; //thread의 priority_before를 priority로 초기화 
...
/* week1-1 alarm 함수 */
bool if_less(struct list_elem *up,struct list_elem *down,void *aux UNUSED){
	if(list_entry(up, struct thread, elem)->wakeup_tick > list_entry(down, struct thread, elem)->wakeup_tick){
		return false;
	}
	return true;
}
void thread_sleep(int64_t ticks){
	...
	ASSERT (!intr_context ());
	old_level = intr_disable ();
	curr ->wakeup_tick = ticks; //수정

	ASSERT(curr != idle_thread);
	list_insert_ordered(&sleep_list, &(curr->elem),if_less, NULL);
	
	// printf("sleep :is empty? =%d tick=%ld\n",list_empty(&sleep_queue),ticks);
...
void thread_awake(int64_t ticks){
	...
	// printf("awake:is empty? =%d tick=%ld\n",list_empty(&sleep_queue),ticks);
	while (list_end(&sleep_list)!=e){
		if (list_entry(e,struct thread,elem)->wakeup_tick>ticks)
			break;
		next = list_remove(e); //sleep_queue에서 지우는거
		thread_unblock(list_entry(e, struct thread, elem)); //문제 예상
		e = next;
	}
	update_next_tick_to_awake();
}
...
void update_next_tick_to_awake(void){
	if (list_empty(&sleep_list)==0){
		next_tick_to_awake = list_entry(list_begin(&sleep_list), struct thread, elem)->wakeup_tick;
	} else{
...
/* week1-2 priority 추가함수 */
// ready_list 에 삽입할 때 list_insert_ordered를 사용하기 위한 비교 함수
...
void test_max_priority(void) {
	...
	if(!list_empty(&ready_list) && thread_current()->priority < list_entry(e, struct thread, elem)->priority) {
		thread_yield();
	}
}
/* week1-2 priority 추가함수 */
...
void donate_priority (void){
	...
	while (cnt<9){
		cnt++;
		if (t->wait_on_lock_p == NULL){
			break;
		}
		t = t->wait_on_lock_p->holder;
...
{
	struct thread *curr = thread_current();
	curr->priority = curr->priority_before;
